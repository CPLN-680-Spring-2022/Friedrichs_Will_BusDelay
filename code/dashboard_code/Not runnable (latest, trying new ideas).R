...
a_routes = routes[1:35,]
b_routes = routes[36:70,]
c_routes = routes[71:105,]
d_routes = routes[106:140,]
...
route_data_4_11 =  read.csv("https://raw.githubusercontent.com/CPLN-680-Spring-2022/Friedrichs_Will_BusDelay/main/raw_data/map_data/route_data_4_11.csv")
route_data_4_12 =  read.csv("https://raw.githubusercontent.com/CPLN-680-Spring-2022/Friedrichs_Will_BusDelay/main/raw_data/map_data/route_data_4_12.csv")
route_data_4_17 =  read.csv("https://raw.githubusercontent.com/CPLN-680-Spring-2022/Friedrichs_Will_BusDelay/main/raw_data/map_data/route_data_4_17.csv")
route_data_4_18 =  read.csv("https://raw.githubusercontent.com/CPLN-680-Spring-2022/Friedrichs_Will_BusDelay/main/raw_data/map_data/route_data_4_18.csv")
route_data_4_19 =  read.csv("https://raw.githubusercontent.com/CPLN-680-Spring-2022/Friedrichs_Will_BusDelay/main/raw_data/map_data/route_data_4_19.csv")
  ...
  tabPanel("Summary Statistics",
    sidebarLayout(
      sidebarPanel(
        fluidRow(
          column(width=6, 
                 selectInput(inputId = "date", label = "", choices = c("April 11, 2022")),
                 selected = "plot")
        )
      ),
      mainPanel(
  ...
  tabPanel("Map",
    sidebarLayout(
      sidebarPanel(
        fluidRow(
          column(width=6, 
                 selectInput(inputId = "date2", label = "", choices = c("April 19, 2022",
                                                                        "April 18, 2022",
                                                                        "April 17, 2022",
                                                                        "April 12, 2022",
                                                                        "April 11, 2022")),
...
server <- function(input, output, session) {
  observe({
    ...
    {
      updateCheckboxGroupInput("group1","checka","",choices=a_routes$lineabbr)
      updateCheckboxGroupInput("group2","checkb","",choices=b_routes$lineabbr)
      updateCheckboxGroupInput("group3","checkc","",choices=c_routes$lineabbr)
      updateCheckboxGroupInput("group4","checkd","",choices=d_routes$lineabbr)
    }
    else
    {
      updateCheckboxGroupInput("group1","checka","",choices=a_routes$lineabbr,selected=a_routes$lineabbr)
      updateCheckboxGroupInput("group2","checkb","",choices=b_routes$lineabbr,selected=b_routes$lineabbr)
      updateCheckboxGroupInput("group3","checkc","",choices=c_routes$lineabbr,selected=c_routes$lineabbr)
      updateCheckboxGroupInput("group4","checkd","",choices=d_routes$lineabbr,selected=d_routes$lineabbr)
    }
  })
  date_data <- reactive({
    if (input$"date" == "April 11, 2022") {
      if (input$"SPANOO" == "Early Arrivals") {
        seleccion = filter(route_data_4_11, c(
          route, pct_earl_trips, sum_earl_hours
          )
        )
      }
      else {
        seleccion = filter(route_data_4_11,  c(
          route, pct_canc_trips, sum_canc_hours
        )
        ) 
      }
    } else if (input$"date" == "April 12, 2022") {
      if (input$"SPANOO" == "Early Arrivals") {
        seleccion = filter(route_data_4_12, c(
          route, pct_earl_trips, sum_earl_hours
        )
        ) 
      }
    ...
    } else if (input$"date" == "April 17, 2022") {
      if (input$"SPANOO" == "Early Arrivals") {
        seleccion = filter(route_data_4_17, c(
          route, pct_earl_trips, sum_earl_hours
        )
        )
      }
    ...
    } else if (input$"date" == "April 18, 2022") {
      if (input$"SPANOO" == "Early Arrivals") {
        seleccion = filter(route_data_4_18, c(
          route, pct_earl_trips, sum_earl_hours
        )
        )
      }
      ...
      if (input$"SPANOO" == "Early Arrivals") {
        seleccion = filter(route_data_4_19, c(
          route, pct_earl_trips, sum_earl_hours
        )
        )
      }
 ...
 #     addCircleMarkers(data = point_data(), weight = 1)  %>%
       addPolylines(data = a_routes, color = 'white', weight = 2,
                    label = ~paste("This is Bus Route ", 
                                   select(date_data(), route == "48")[1]), # note the tilde / ~
                    labelOptions = labelOptions(noHide = F, direction = "top"),
                    group = "group1") %>%
      addPolylines(data = b_routes, color = 'white', weight = 2,
                   label = ~paste("This is Bus Route ", 
                                  select(date_data(), route == "48")[1]), # note the tilde / ~
                   labelOptions = labelOptions(noHide = F, direction = "top"),
                   group = "group2") %>%
      addPolylines(data = c_routes, color = 'white', weight = 2,
                   label = ~paste("This is Bus Route ", 
                                  select(date_data(), route == "48")[1]), # note the tilde / ~
                   labelOptions = labelOptions(noHide = F, direction = "top"),
                   group = "group3") %>%
      addPolylines(data = d_routes, color = 'white', weight = 2,
                   label = ~paste("This is Bus Route ", 
                                  select(date_data(), route == "48")[1]), # note the tilde / ~
                   labelOptions = labelOptions(noHide = F, direction = "top"),
                   group = "group4")
  ...
  mydata_filtered1 <- reactive(a_routes[a_routes$lineabbr %in% input$group1, ])
  mydata_filtered2 <- reactive(b_routes[b_routes$lineabbr %in% input$group2, ])
  mydata_filtered3 <- reactive(c_routes[c_routes$lineabbr %in% input$group3, ])
  mydata_filtered4 <- reactive(d_routes[d_routes$lineabbr %in% input$group4, ])
  
  
  observeEvent(input$checkGroup, {
    leafletProxy("mymap", data = mydata_filtered1()) %>%
      clearGroup ("group1") %>%
      addPolylines(color = 'white', weight = 2,
                   label = ~paste("This is Bus Route ", 
                                  select(date_data(), route == "48")[1]), # note the tilde / ~
                   labelOptions = labelOptions(noHide = F, direction = "top"),
                   group = "group1")
  ...
  observeEvent(input$checkGroup2, {
    leafletProxy("mymap", data = mydata_filtered2()) %>%
      clearGroup ("group2") %>%
      addPolylines(color = 'white', weight = 2,
                   label = ~paste("This is Bus Route ", 
                                  select(date_data(), route == "48")[1]), # note the tilde / ~
  ...
  observeEvent(input$checkGroup, {
    leafletProxy("mymap", data = mydata_filtered3()) %>%
      clearGroup ("group3") %>%
      addPolylines(color = 'white', weight = 2,
                   label = ~paste("This is Bus Route ", 
                                  select(date_data(), route == "48")[1]), # note the tilde / ~
  ...
  observeEvent(input$checkGroup2, {
    leafletProxy("mymap", data = mydata_filtered4()) %>%
      clearGroup ("group4") %>%
      addPolylines(color = 'white', weight = 2,
                   label = ~paste("This is Bus Route ", 
                                  select(date_data(), route == "48")[1]), # note the tilde / ~
